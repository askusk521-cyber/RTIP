...
def _repulsive_md_bias(config: RepulsivePot, system: System, step: int) -> tuple[float, float, Any, float]:
...
def _attractive_md_bias(config: AttractivePot, system: System, step: int) -> tuple[float, float, Any, float]:
...
def _synthesis_md_bias(config: SynthesisPot, system: System, step: int) -> tuple[float, float, Any, float]:
...
def _md_bias(config: MDConfig, system: System, step: int) -> tuple[float, float, Any, float]:
    if isinstance(config, RepulsivePot):
        return _repulsive_md_bias(config, system, step)
    if isinstance(config, AttractivePot):
        return _attractive_md_bias(config, system, step)
    if isinstance(config, SynthesisPot):
        return _synthesis_md_bias(config, system, step)
