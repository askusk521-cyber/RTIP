...
@dataclass(frozen=True)
class EvolutionPot:
    """Configuration for the formose-style evolution MD (Rust EvolutionPot).

    An attractive RTIP pulls the current system toward a virtual final state
    in which all molecular centroids coincide (moved by 0.01 per step).  The
    RTIP depth follows the three-state machine Increasing / Decreasing /
    Falling, driven by bond-variation detection.
    """
    ...
    str_output_file: str = "rtip.pdb"
    output_file: str = "rtip.out"
    dec_output_file: str = "rtip_decreasing_steps"
